...
export default function Checkout() {
  ...
  const handleWhatsApp = () => {
    ...
    const items = getCartSummary().map((i) => `${i.quantity}x ${i.name}`).join(", ");
    const msg = encodeURIComponent(`Hello, I'd like to pay for my order: ${items}. Total: ₦${totalPrice.toLocaleString()}.`);
  ...
  const handlePaystack = async () => {
    try {
      setProcessing(true);
      const backendUrl = process.env.NEXT_PUBLIC_API_URL || "https://that.amv.local";

      const { data } = await axios.post(`${backendUrl}/api/payments/initialize`, {
        email: "customer@example.com", // Replace with real email input if available
        amount: totalPrice,
    ...
    return (
      <div className="min-h-[70vh] flex items-center justify-center bg-white">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm text-gray-500 uppercase tracking-widest">Loading Cart...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-[80vh] bg-white flex flex-col items-center justify-center px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center max-w-md">
          ...
          <p className="text-gray-500 mb-8 font-light">You haven't added any luxury items yet.</p>
          <Link href="/shop" className="inline-flex items-center justify-center px-8 py-4 text-xs font-bold uppercase tracking-widest bg-black text-white hover:bg-gray-800 transition-colors rounded-sm">
            Start Shopping
          </Link>
        </motion.div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 lg:py-20">
        ...
        <div className="flex items-center justify-between border-b border-gray-100 pb-6 mb-12">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-bold uppercase tracking-widest">Secure Checkout</h1>
          </div>
          <Link href="/" className="text-sm text-gray-500 hover:text-black flex items-center gap-2 transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20">
          ...
          <div className="lg:col-span-7">
            <h2 className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-6">Order Details</h2>
            <div className="space-y-8">
              {cart.map((item) => {
                const p = item.product;
                const img = p.image?.startsWith("http") ? p.image : "/placeholder.jpg";
                return (
                  <motion.div key={item.product._id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-6 border-b border-gray-50 pb-8 last:border-0">
                    ...
                    <div className="flex-1 flex flex-col justify-between py-1">
                      <div>
                        <div className="flex justify-between items-start">
                          ...
                          <span className="text-sm font-semibold">{formatPrice(p.price * item.quantity)}</span>
                        </div>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <p className="text-xs text-gray-400">Unit: {formatPrice(p.price)}</p>
                      </div>
                    </div>
                  </motion.div>
                );
          ...
          <div className="lg:col-span-5">
            <div className="bg-gray-50 p-8 lg:p-10 rounded-sm sticky top-10">
              ...
              <div className="mt-8 pt-6 border-t border-gray-200 flex items-center justify-center gap-2 text-gray-400">
                ...
                <span className="text-[10px] uppercase tracking-widest">Encrypted Payment Processing</span>
              </div>
            </div>
          </div>
        </div>
